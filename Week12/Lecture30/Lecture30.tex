...
\include{Lecture30_announcements}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Sections
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{center}
\includegraphics[width=0.5\textwidth]{\chpvii@path/7-5_anova/figures/aldrin/wolf}
\end{center}
...
\includegraphics[width=0.9\textwidth]{\chpvii@path/7-5_anova/figures/aldrin/f}
...
If $\alpha=0.05$:
\begin{itemize}
\item \dq{What is the chance that I get a type I error when doing 1 test?} \soln{\only<2-5>{ 0.05 (5\%)  -- definition of $\alpha$} }
\item \dq{How many Type I errors would I expect if I did 100 independent tests with no true effects?} \soln{\only<3-5>{$X\sim Binomial(100, 0.05)$ so $E[X]=np=5$} }
\item \dq{What is the probability of at least one Type I error if I do 100 independent tests with no true effects?} \soln{\only<4-5>{ $1 - (1 - 0.05)^{100} \approx 0.994$ (99.4\%) } }
...
\begin{frame}
\frametitle{Which means differ? Post-hoc t-tests}

\pq{Based on the box plots below, which means would you expect to be significantly different?}

% \twocol{0.6}{0.4}{
\includegraphics[width=\textwidth]{\chpvii@path/7-5_anova/figures/aldrin/homo}
% }
% {
% \begin{enumerate}[(a)]
% \item bottom \& surface
...
\begin{frame}
\frametitle{Which means differ? Post-hoc t-tests (cont.)}
...
\pause
p-value &=& \texttt{2*pt(1.87,27,lower.tail=FALSE)} = 0.072 \quad \text{{\footnotesize (two-sided)}} \\
\pause
...
We found that the average aldrin concentration at bottom and surface are significantly different, but bottom and mid-depth are not.
\vspace{-1em}
\begin{center}
\includegraphics[width=0.7\textwidth]{\chpvii@path/7-5_anova/figures/aldrin/homo}
\end{center}
\vspace{-1em}
\Note{Don't be tempted to compare the p-values or hypothesis test results! If you want to compare effects, design a hypothesis test for that.}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item How to group attractiveness ratings (e.g., 5 vs 1-4, or other splits)? Why didn't they use an ANOVA?\pause
        \item Whether to include/exclude certain respondents or outliers.\pause
        \item Choice of outcome variable (first child, all children, etc.).\pause
        \item Selection of statistical tests and transformations.\pause
...
\begin{itemize}
    ...
    \item Use Bonferroni or other corrections for multiple comparisons
...
\section{Preview: Linear Regression}
...
\twocol{0.55}{0.45}{
\begin{center}
\includegraphics[width=\textwidth]{\chpviii@path/8-1_linefit_res_corr/figures/poverty/poverty_hsgrad}
\end{center}
}
{
