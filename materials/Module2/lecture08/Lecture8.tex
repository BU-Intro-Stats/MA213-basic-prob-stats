...
\include{Lecture8_announcements}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Worksheet: Bayes' Theorem}

\begin{frame}
\frametitle{Worksheet: Bayes' Theorem}

Pick up the worksheet and find a partner.

\begin{itemize}
    \item \textbf{Step 1 (Think, 2 min):} apply the Law of Total Probability and Bayes' Theorem to a medical testing scenario.
    \item \textbf{Step 2 (Pair, 2 min):} compare answers with your partner and discuss what the result implies about screening for rare diseases.
\end{itemize}
...
\begin{frame}
\frametitle{Sampling with vs.\ without replacement}

Imagine a bag with 5 red, 3 blue and 2 orange chips (10 total):\\
5 \textcolor{red}{$\CIRCLE$}~, 3 \textcolor{blue}{$\CIRCLE$}~, 2 \textcolor{orange}{$\CIRCLE$},
so $Prob(B_1) = \frac{3}{10} = 0.3$.

\pause

Suppose the first chip drawn is blue. What is $Prob(B_2 | B_1)$?

\pause
\vspace{1mm}

\twocol{0.5}{0.5}{
\centering \textbf{With replacement} \\ {\footnotesize (chip goes back in)}
\begin{center}
5 \textcolor{red}{$\CIRCLE$}~, 3 \textcolor{blue}{$\CIRCLE$}~, 2 \textcolor{orange}{$\CIRCLE$}
\end{center}
\[ Prob(B_2|B_1) = \frac{3}{10} = 0.3 \]
}{
\centering \textbf{Without replacement} \\ {\footnotesize (chip stays out)}
\begin{center}
5 \textcolor{red}{$\CIRCLE$}~, 2 \textcolor{blue}{$\CIRCLE$}~, 2 \textcolor{orange}{$\CIRCLE$}
\end{center}
\[ Prob(B_2|B_1) = \frac{2}{9} \approx 0.22 \]
}
...
{\footnotesize
\begin{itemize}
\item With replacement, the bag never changes, so $Prob(B_2|B_1) = Prob(B_1)$: \hl{draws are independent}.
\item Without replacement, the bag's composition changes, so $Prob(B_2|B_1) \ne Prob(B_1)$: \hl{draws are not independent}.
\end{itemize}
}
...
\begin{frame}
\frametitle{Probability of drawing two blues in a row}

\[ Prob(B_1) \cdot Prob(B_2 | B_1) \]

\pause
\vspace{3mm}

\twocol{0.5}{0.5}{
\centering \textbf{With replacement}
\[ 0.3 \times 0.3 = 0.09 \]
}{
\centering \textbf{Without replacement}
\[ 0.3 \times 0.22 = 0.066 \]
}
...
The gap between the two matters most when the population is \hl{small}: removing one chip out of 10 changes the odds a lot.
...
\begin{frame}
\frametitle{Sampling without replacement from a large population}

Now suppose the bag has 5000 red, 3000 blue and 2000 orange chips (same proportions, so $Prob(B_1) = 0.3$ still). Given the first chip drawn is blue:

\vspace{2mm}

\twocol{0.5}{0.5}{
\centering \textbf{With replacement}
\[ Prob(B_2|B_1) = \frac{3000}{10000} = 0.3 \]
}{
\centering \textbf{Without replacement}
\[ Prob(B_2|B_1) = \frac{2999}{9999} \approx 0.29997 \]
}

\pause
\vspace{4mm}

With a large population, removing one chip barely changes the bag, so draws without replacement are \hl{approximately independent.}
