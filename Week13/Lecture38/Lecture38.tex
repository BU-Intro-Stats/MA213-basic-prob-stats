...
\item \textbf{Behavior}: Cursor position and hand kinematics (position/velocity) over time
\end{itemize}
...
\textbf{Goal}: Predict cursor position from neural firing rates
...
\begin{itemize}
\item We used linear regression to decode cursor position from neural activity. \pause
\item Even when the model is massively simplified (and incorrect), it can still provide useful predictions. \pause
\item We often start with simple models to build intuition before moving to more complex approaches.\pause
\item This math is the foundation for modern machine learning methods used in brain-computer interfaces.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Ways that this model could be improved}

\begin{itemize}
    \item Model the \hl{hand kinematics} (position, velocity) instead of cursor position alone.\pause
    \item Use the spikes themselves as predictors instead of smoothed firing rates. To get the noise model right, we could use a \hl{Poisson distribution}. Regression with Poisson responses is an example of a \hl{generalized linear model (GLM)}.\pause
    \item Capture the \hl{temporal dependencies} in the data using \hl{state space modeling}.\pause
    \item Use \hl{regularization} (e.g., ridge regression, LASSO) to prevent overfitting given the large number of predictors.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{If you're interested in this topic...}

\begin{itemize}
    \item \textbf{Neural Latents Benchmark}: \webLink{https://neurallatents.github.io/}{https://neurallatents.github.io/} - Public datasets and benchmarks for neural decoding.
    \item \textbf{BrainGate}: \webLink{https://braingate.org/}{https://braingate.org/} - Research consortium developing clinical BCIs.
    \item \textbf{Chang Lab}: \webLink{https://changlab.ucsf.edu}{https://changlab.ucsf.edu} - Leading research group on speech BCIs.
    \item \textbf{Articles}:
    \begin{itemize}
        \item Pei et al. (2021) \textit{Neural Latents Benchmark '21: Evaluating latent variable models of neural population activity. } Biorxiv. \webLink{https://doi.org/10.48550/arXiv.2109.04463}{https://doi.org/10.48550/arXiv.2109.04463}
        \item Chen et al. (2025) \textit{Brain-computer interfaces in 2023-2024.} Brain-X. \webLink{https://doi.org/10.1002/brx2.70024}{https://doi.org/10.1002/brx2.70024}
        \item Karpowicz et al. (2025) \textit{Stabilizing brain-computer interfaces through alignment of latent dynamics.} Nature Communications. \webLink{https://doi.org/10.1038/s41467-025-59652-y}{https://doi.org/10.1038/s41467-025-59652-y}
    \end{itemize}
\end{itemize}
\end{frame}
