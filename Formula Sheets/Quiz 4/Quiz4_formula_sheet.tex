...
\begin{align*}
    &\frac{\bar{X}-\mu}{s/\sqrt{n}} \sim t_{df} \\
    & df = n - 1
\end{align*}
...
Special cases:
\begin{itemize}
    \item For single/two proportion CIs,  when computing SE, can substitute $\hat{p}$ for $p$
\end{itemize}

Computing sample size for desired ME:
\begin{itemize}
    \item Write down the ME formula in terms of $n$, $z^*$ and/or $t^*$, and other known quantities, and solve for $n$
\end{itemize}
...
\begin{align*}
    &X^2 = \sum_i^k \frac{(O_i - E_i)^2}{E_i} \sim \chi^2_{df} \\
    & df = k - 1 
\end{align*}
...
% Common critical values for HT table (save for Quiz 4)
