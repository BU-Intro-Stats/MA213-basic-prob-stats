...
\include{Lecture28_agenda}
\include{Lecture28_announcements}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Sections
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{columns}
		...
		\column{0.44\textwidth}
			...
			\begin{itemize}
				...
				\item Normal approx. $np > 15$ and $n(1-p) > 15$ 
...
\begin{frame}
	...
	\end{align*}
\pause
	Using \texttt{qnorm} in R, $\Pr(Z > 2.4) = 0.0075$
\pause
	\begin{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
	\frametitle{Worksheet: Sex Ratios}
	\begin{center}
		\includegraphics[height=0.5\textheight]{figures/whatif_sexratio.png}
	\end{center}
	\footnotesize{
	\begin{itemize}
		\item Approximately how big did the difference in sex ratios have to be to reach significance?
		\item Approximately what was the power of the test to detect an effect of 1\%?
		\item If you reject the null hypothesis, what is the approximate probability that your observed difference in sex ratios was in the wrong direction (sign error)?
	\end{itemize}
	}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item The difference in sex ratios had to be at least \hl{6.5\%} to reject the null hypothesis (but a realistic effect size would be less than 1\%)
...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item If you suspect that a study is underpowered, be very cautious when interpreting statistically significant results
		\begin{itemize}
			\item Don't trust large effect sizes
			\item Don't trust the direction of the effect
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Never calculate power post-hoc based on the effect size from the current experiment
