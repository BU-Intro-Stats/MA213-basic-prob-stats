...
\include{Lecture23_announcements}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Sections
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}
\frametitle{Recap: Chi-square Goodness of Fit Test}
\begin{enumerate}
  \item \hl{Conditions:}
    \begin{itemize}
      \item[$\checkmark$] Independence: The cases (rows) must be independent.
      \item[$\checkmark$] Sample size: Each expected count must be at least 5.
      \item[$\checkmark$] Degrees of freedom $>$ 1 (so at least 3 categories).
    \end{itemize}
  \pause
  \item \hl{Setting the hypotheses:} 
    \begin{itemize}
      \item[$H_0$:] The observed counts follow the expected proportions.
      \item[$H_A$:] The observed counts do not follow the expected proportions.
    \end{itemize}
  \pause
  \item \hl{Calculating the test statistic:}
    \[
    \chi^2 = \sum_{i = 1}^k \frac{(O - E)^2}{E} \qquad \text{where $k$ = total number of categories}
    \]
  \pause
  \item Under $H_0$, $\chi^2$ follows a chi-square distribution with $df = k - 1$.
  \pause
  \item \hl{Calculating the p-value:} The p-value is the area under the $\chi^2_{df}$ curve, above the calculated test statistic.
\end{enumerate}
...
\dq{There was lots of talk of election fraud in the 2009 Iran election. We'll compare the data from a poll conducted before the election (observed data) to the reported votes in the election to see if the two have the same proportions.}
...
\dq{What are the hypotheses for testing if the proportions of reported and polled votes are different?}

\soln{
\only<2>{
\begin{itemize}
\item[$H_0$:] The observed counts from the poll have the same proportions as the reported votes.
\item[$H_A$:] The observed counts from the poll do not have the same proportions as the reported votes.
\end{itemize}
}}
...
\begin{enumerate}[(a)]
\solnMult{p-value is low, $H_0$ is rejected. The observed counts from the poll do \underline{not} have the same proportions as the reported votes.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Edfinity Quiz: Independence}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Edfinity Quiz: Computing expected counts}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\section{Edfinity Quiz: Chi square p-value}
...
\section{Edfinity Quiz: Conclusion}
...
\section{(Preview) Chapter 7: Inference for numerical data}
