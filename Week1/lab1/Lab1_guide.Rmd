...
setwd('path of your working directory') # replace with your own path
...
getwd() # prints the path of current working directory
...
```{r, echo=TRUE}
SchoolName <- "Boston University" # assign text string
SchoolName # print the value
```
...
```{r, echo=TRUE}
GPA <- c(3.2, 3.7, 3.9, 2.3, 2.7) # numeric vector
FirstName <- c("Tom", "Sarah", "Nick", "Amy", "John") # character vector
GPA # print the GPA vector
FirstName # print the FirstName vector
...
```{r, echo=TRUE}
my_vector = seq(1, 10, 2) # numbers from 1 to 10 with step size 2
my_vector # print the vector

```

```{r, echo=TRUE}
#Shortcut to create vector from 1 to 10
another_vector = 1:10
another_vector
...
```{r, echo=TRUE}
#Replicate an element multiple times
vector3 = rep("BU", 25) # repeat "BU" 25 times
vector3
...
```{r, echo=TRUE}
my_data$hp < 100 # returns TRUE/FALSE for each row
my_data[my_data$hp < 100, ] # shows only rows where hp < 100
```
