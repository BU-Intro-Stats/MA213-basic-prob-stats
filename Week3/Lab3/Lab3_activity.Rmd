...
3.  Obtain the scatter plot of `B` and `C` . What kind of relationship can you see? You can choose from those two
...
4.  You are interested in looking at this mysterious variable `D` in the data. Look at the distribution of the data by producing a plot, answer if the shape of `D` is one of
    ...
    You can answer this by assigning a variable `shape` . (For example, you can do `shape <- "right skew"` or \``shape <- "uniform"` ... )
