# Syllabus Course Information

<!--
Instructor notes:
- Syllabus: this file controls the course description, materials, and technology sections.
- Course title and semester: the generator should use the course configuration and important_dates.md.
- Meeting days and times: the generator should use Lecture_schedules.md and Lab_schedules.md.
- Review the platform names, prices, and enrollment instructions before each semester.
-->

## Course Details

| Field | Value |
| --- | --- |
| Course Name | Basic Statistics and Probability |
| Institution | Boston University |
| Learning Management System | Blackboard |
| Lecture Location | PHO 206 |
| Discussion and Lab Locations | Check your schedule. |

## Course Description

This course serves as an introduction to basic concepts and tools in probability and statistics. We begin with techniques for describing data. Then we study the elements of probability theory, including probability densities, means, variances, correlation, independence, the binomial distribution, and the central limit theorem. Finally, we combine data description and probability theory into an approach to statistical inference. Students should emerge from this course with the ability to incorporate a variety of skills in analyzing and reasoning from data.

## Course Materials

| Item | Required | Details | URL |
| --- | --- | --- | --- |
| TEXTBOOK: OpenIntro Statistics, 4th Edition | Yes | Open-source textbook by {{Ce\-tin\-kaya-\allowbreak Run\-del}}, Diez, and Barr. A PDF is available at no cost; optional print copies are available from the publisher. | https://www.openintro.org/book/os/ |
| Edfinity | Yes | Used for in-class activities and weekly auto-graded homework. The Fall 2026 reference price was $40 with OfficeHours. |  |
| Laptop, phone, or similar device | Yes | Bring a device to lectures to participate in in-class activities. |  |
| Pen or pencil | Yes | Bring a pen or pencil to lectures and labs to complete the worksheets. |  |
| R and RStudio | Yes | Used during skills labs for statistical programming and data analysis. | https://posit.co/download/rstudio-desktop/ |
| Gradescope | Yes | Used to submit lab deliverables and revisions. |  |

## Edfinity Enrollment

1. Use a current version of Google Chrome or Firefox on a Windows or Mac computer. Other browsers or devices may cause issues when Edfinity is accessed through Blackboard.
2. If you already have an Edfinity account, sign out of Edfinity before beginning enrollment.
3. Log in to Blackboard (learn.bu.edu).
4. Open the Edfinity link for this course in Blackboard.
5. Follow the prompts to enroll and pay the course fee.
