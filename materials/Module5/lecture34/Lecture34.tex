...
\begin{frame}
    \frametitle{Think/Pair/Share: Updating a Beta Prior}
    Pick up the worksheet and find a partner.
    \medskip
    \begin{itemize}
        \item \textbf{Step 1 (Think, 5 min):} apply the Beta-Binomial update rule yourself to find a posterior, then see how a different prior changes the answer.
...
\begin{frame}[fragile]
    \frametitle{Visualizing a Credible Interval}
    For the drug trial posterior, $p\mid x \sim Beta(3,9)$:
    \begin{center}
        \includegraphics[width=0.68\textwidth]{credible_interval.pdf}
    \end{center}
    \pause
    An \hl{equal-tailed} interval leaves 2.5\% of the posterior probability in each tail --- so $a$ and $b$ are just the 2.5th and 97.5th percentiles of the posterior.
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Edfinity Quiz: Computing a Credible Interval in R}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}
    \pq{Recall the drug trial posterior, $p\mid x \sim Beta(3,9)$. Which line of R code correctly computes the 95\% equal-tailed credible interval?}

    \small
    \begin{enumerate}[(a)]
        \solnMult{\texttt{qbeta(c(0.025, 0.975), 3, 9)}}
        \item \texttt{qbeta(c(0.05, 0.95), 3, 9)}
        \item \texttt{dbeta(c(0.025, 0.975), 3, 9)}
        \item \texttt{dbeta(c(0.05, 0.95), 3, 9)}
    \end{enumerate}
\end{frame}
